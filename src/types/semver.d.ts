...
// Updated for semver 5.7, to be replaced with newer version of @types/semver once available
...
declare module 'semver' {
  ...
  /**
   * Returns cleaned (removed leading/trailing whitespace, remove '=v' prefix) and parsed version,
   * or null if version is invalid.
   */
  export function clean(version: string, loose?: boolean): string|null;
  /**
   * Return the version incremented by the release type (major, minor, patch, or prerelease), or
   * null if it's not valid.
   */
  export function inc(
      v: string|SemVer, release: ReleaseType, loose?: boolean, identifier?: string): string|null;
  /**
  ...
  /**
   * v1 == v2 This is true if they're logically equivalent, even if they're not the exact same
   ...
   */
  export function cmp(
      v1: string|SemVer, operator: Operator, v2: string|SemVer, loose?: boolean): boolean;
  export type Operator = '==='|'!=='|''|'='|'=='|'!='|'>'|'>='|'<'|'<=';

  /**
   * Return 0 if v1 == v2, or 1 if v1 is greater, or -1 if v2 is greater. Sorts in ascending order
   * if passed to Array.sort().
   */
  export function compare(v1: string|SemVer, v2: string|SemVer, loose?: boolean): 1|0|- 1;
  /**
   * The reverse of compare. Sorts an array of versions in descending order when passed to
   * Array.sort().
   */
  export function rcompare(v1: string|SemVer, v2: string|SemVer, loose?: boolean): 1|0|- 1;

  /**
   * Compares two identifiers, must be numeric strings or truthy/falsy values. Sorts in ascending
  ...
  export function maxSatisfying(
      versions: Array<string|SemVer>, range: string|Range, loose?: boolean): string;
  /**
   * Return the lowest version in the list that satisfies the range, or null if none of them do.
   */
  export function minSatisfying(
      versions: Array<string|SemVer>, range: string, loose?: boolean): string;
  /**
   * Return the lowest version that can possibly match the given range.
   */
  export function minVersion(range: string): SemVer;
  /**
   ...
   * direction. The hilo argument must be either the string '>' or '<'. (This is the function called
   * by gtr and ltr.)
   */
  export function outside(
      version: string|SemVer, range: string|Range, hilo: '>'|'<', loose?: boolean): boolean;
  /**
